...
PROTOC_GEN_GO_PULSAR := $(GOPATH)/bin/protoc-gen-go-pulsar
PROTO_PULSAR_IMPORTS := \
	--proto_path=proto \
	--proto_path=$(GOPATH)/pkg/mod/github.com/cosmos/cosmos-sdk@v0.53.4/proto \
	--proto_path=$(GOPATH)/pkg/mod/github.com/cosmos/cosmos-proto@v1.0.0-beta.5/proto \
	--proto_path=$(GOPATH)/pkg/mod/github.com/cosmos/gogoproto@v1.7.0 \
	--proto_path=$(GOPATH)/pkg/mod/github.com/grpc-ecosystem/grpc-gateway@v1.16.0/third_party/googleapis \
	--proto_path=x/dex/proto
...
# Pulsar API generation is intentionally disabled because the generated output
# in api/marketplace is not used by the current module and breaks builds.
