...
export default function BuyMastercard() {
  ...
  const start = (e) => {
    ...
    setTimeout(() => {
      setProviderRef(`MC-${Math.random().toString(36).slice(2, 10).toUpperCase()}`)
      setStep('success')
    }, 4200)
  }
  ...
  return (
    <div className="max-w-lg mx-auto space-y-8">
      <div>
        ...
        <h1 className="text-3xl font-black">Mastercard</h1>
        <p className="text-slate-400 mt-2">Mastercard 3D Secure flow (demo UI).</p>
      </div>
      ...
      >
        ...
        <button
          type="submit"
          className="w-full py-4 rounded-2xl bg-gradient-to-r from-red-500 to-orange-500 font-bold flex items-center justify-center gap-2"
        >
      ...
      {step === 'processing' && (
        <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 text-center space-y-4">
          <div className="mx-auto w-12 h-12 border-4 border-orange-500 border-t-transparent rounded-full animate-spin" />
          <h2 className="text-xl font-bold">Securing your connection with bank</h2>
          <p className="text-slate-400 text-sm">OTP verification will complete the payment.</p>
        </div>
      )}
